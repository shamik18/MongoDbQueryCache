...
public class WhereClauseBuilder {
    public WhereClause populateWhereClause(Object jsonWhere) {
        ...
        if(!(jsonWhere instanceof List)){
            throw new RuntimeException("Not a valid where clause json node");
        }
        List<JsonNode> nodes = (List<JsonNode>) jsonWhere;
                            ...
                            if(elem instanceof  JsonToken ){
                                ...
                                if(tokenElem.equals(JsonToken.START_OBJECT)){
                                    if(previousValue instanceof  Operands){
                                        ...
                                        Attribute a = createAttribute(attr,operands);
                                        if(!OperatorEnum.EXIST.equals(operands.operatorEnum())){
                                            stack.push(a);
                                        }
                                        if(!operandSet.isEmpty()){
                                            for (Operands op: operandSet){
                                                a = createAttribute(attr,op);
                                                if(!op.operatorEnum().equals(OperatorEnum.EXIST)){
                                                    stack.push(a);
                                                }
                                            }
                                        }
                                    }else if(previousValue instanceof  String ||
                                            previousValue instanceof  Integer ||
                                            previousValue instanceof  Boolean ||
                                            previousValue instanceof  Long){
//                                        Attribute attr = null;
//                                        if(stack.peek() instanceof  Attribute)
//                                            attr = (Attribute) stack.pop();
//                                        Operands
                                ...
                                if(elem == null){
                                    ...
                                    if(previousValue == null)
                                        ...
                                        operandSet.add((Operands) previousValue);
                                        previousValue = elem;
                                    }else if(previousValue instanceof Attribute){
                                        attrubuteSet.add((Attribute)previousValue);
                                        previousValue = elem;
                                    }
                                }
            ...
            if(stack.size() == 1){
                ...
                if(ob instanceof Constrain){
                    if(whereClause.getConstrain()!= null && !whereClause.getConstrain().getConstrain_name().equals("and")){
                        Constrain andConstrain = new Constrain();
                        Set<Token> constrainSet = new HashSet<>();
                        constrainSet.add((Constrain) ob);
                        andConstrain.setChild(constrainSet);
                        ob = andConstrain;
                    }
